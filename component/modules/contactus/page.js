...
const ContactUsPopup = ({ onClose }) => {
  ...
  const handleCallClick = useCallback(() => {
    ...
    const newUrl = `${window.location.origin}${window.location.pathname}${window.location.pathname.endsWith('/') ? '' : '/'}call-us`;
    ...
    setTimeout(() => {
      if (window.location.pathname.endsWith('/call-us')) {
        const cleanUrl = window.location.href.replace('/call-us', '');
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (!document.hidden && window.location.pathname.endsWith('/call-us')) {
        const cleanUrl = window.location.href.replace('/call-us', '');
        window.history.replaceState({}, '', cleanUrl);
      }
    };
