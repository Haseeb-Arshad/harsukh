...
const Floor = ({ imageName, imageLink }) => {
    ...
    if (viewerRef.current) {
      ...
      viewer.addHandler('open', function() {
        if (imageName === 'thirdFloor') {
          ...
          //   const polygon = svgOverlay.querySelector(`polygon[data-image="${apartment['Apartmentno']}"]`);
          //   if (polygon) 
          //   {
          ...
          //     const point = viewer.viewport.imageToViewportCoordinates(centerX, centerY);
          //     const pixelPoint = viewer.viewport.viewportToViewerElementCoordinates(point);
             

          //     button.style.left = `${pixelPoint.x}px`;
          //     button.style.top = `${pixelPoint.y}px`;
          ...
          //   svgOverlay.querySelectorAll('.apartmentButton').forEach(button => {
          //     const polygon = svgOverlay.querySelector(`polygon[data-image="${button.dataset.apartmentno}"]`);
          //     if (polygon) {
          //       updateButtonPosition(button, polygon);
          //     }
          //   });
          // });

    
          // // Add click event listeners to polygons and buttons
          // svgOverlay.querySelectorAll('polygon, .apartmentButton').forEach(element => {
          //   element.addEventListener('click', (e) => {
          //     e.preventDefault();
          //     e.stopPropagation();
          //     const dataTip = element.getAttribute('data-image') || element.parentElement.getAttribute('data-image');
          //     if (dataTip) {
          //       const apartment = floorApartments.find(apt => apt['Apartment No'].toString() === dataTip);
          //       if (apartment) {
          //         setActivePolygon({
          //           id: dataTip,
          //           bedroom: apartment.Bedrooms,
          //           netArea: apartment.Area,
          //           grossArea: apartment.Area,
        ...
        if ( imageName === 'secondFloor') {
          ...
          //     button.className = styles.apartmentButton;
          //     button.style.position = 'absolute';
          //     button.dataset.apartmentno = apartment['Apartmentno'];
  ...
  // function updateButtonPosition(button, polygon) {
  //   // Calculate the topmost point of the polygon
  ...
  //   // Convert SVG coordinates to viewport coordinates
  //   const point = viewer.viewport.imageToViewportCoordinates(topX, topY);
  //   const pixelPoint = viewer.viewport.viewportToViewerElementCoordinates(point);
    
  //   // Position the button slightly above the topmost point
  //   button.style.left = `${pixelPoint.x}px`;
  //   button.style.top = `${pixelPoint.y - 20}px`; // 20px above the topmost point
  ...
  <polyline
   ...
   <polygon data-image="82" data-tip="penthouse06" fill=#006d77 class="${styles.st0}" points="429.49 169.96 429.49 198.81 453.49 198.81 454.21 198.81 454.21 465.3 410.6 465.3 410.6 451.57 321.53 451.57 321.53 276.34 300.77 276.34 300.77 240.25 354.55 240.25 354.55 193.96 354.55 193.45 429.49 169.96"/>
        ...
        if ( imageName === 'basement5') {
          ...
          // Add click event listeners to polygons
             svgOverlay.querySelectorAll('polygon').forEach(element => {
        element.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          const dataTip = element.getAttribute('data-image') || element.parentElement.getAttribute('data-image');
    ...
    // <Suspense fallback={<div className={styles.loadingOverlay}><Loading /></div>}>
      ...
      <div className={styles.floorContainer}>
        ...
        >
          ...
          <h3>Apartment {activePolygon['Apartment No']}</h3>
          <p>Bedrooms: {activePolygon.Bedrooms}</p>
          <p>Area: {activePolygon.Area}</p>
          <p>Type: {activePolygon.Type}</p>
        </div>
      )}
